...
[ApiController]
[Route("api/invoices/{id}/draft")]
public sealed class InvoiceDraftsController(SaveInvoiceDraftService saveService) : ControllerBase
{
    [HttpPut]
    ...
    public async Task<IActionResult> Save(
        string id,
        SaveInvoiceDraftRequest request,
        CancellationToken cancellationToken)
    {
        ...
        {
            SaveInvoiceDraftResult.Saved saved => Ok(saved.Invoice.ToDto(saved.Corrections)),
            SaveInvoiceDraftResult.NotFound => Problem(
                StatusCodes.Status404NotFound,
                "INVOICE_NOT_FOUND",
                "Invoice not found",
                "The requested invoice does not exist."),
            SaveInvoiceDraftResult.VersionConflict conflict => Problem(
                StatusCodes.Status409Conflict,
                "INVOICE_VERSION_CONFLICT",
                "The invoice has changed",
                "Refresh the invoice and reconcile your unsaved changes before trying again.",
                currentVersion: conflict.CurrentVersion.Value),
            SaveInvoiceDraftResult.StateConflict conflict => Problem(
                StatusCodes.Status409Conflict,
                "INVOICE_STATE_CONFLICT",
                "The invoice cannot be edited",
                "The invoice's current state does not allow draft changes.",
                currentVersion: conflict.CurrentVersion.Value),
            _ => throw new InvalidOperationException("The draft-save result is unsupported."),
        };
    }
